...
ADMIN_ID = 123456789  # 1827360709
PREMIUM_PRICE = 150  # Звёзд
FREE_LIMIT = 3  # Бесплатных вакансий в день
...
INFO, format="%(asctime)s | %(levelname)-8s | %(message)s")
            ...
            try:
                ...
                for item in items[:3]:
                    try:
                        ...
                        if any(word in title.lower() for word in ["14", "15", "16", "17", "школьник", "студент", "подработк", "промоутер", "курьер", "раздач", "расклейк"]):
                            vacancies.append({
                                "title": title,
        ...
        for item in data.get("items", []):
            try:
                ...
                if salary:
                    ...
                    payment = f"{salary_from or '?'} - {salary_to or '?'} {salary.get('currency', '₽')}"
                else:
                    payment = "Не указана"
...
async def premium_info(call: CallbackQuery):
    await call.message.edit_text(
        ...
        f"💰 *{PREMIUM_PRICE} звёзд Telegram (~149₽)*\n"
